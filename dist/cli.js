...
function run() {
  _commander.default.version(_package.version).option('-c, --config <path>', 'Sets the path to the config file, otherwise read from the env variable CONFIG_FILE.').parse(process.argv);

  const opts = _commander.default.opts(); // If no config option is given, try to use the env variable:
