...
class Bot {
  ...
  sendToIRC(message) {
    ...
    if (ircChannel) {
      ...
      if (this.ircNickColor) {
        ...
        displayUsername = _ircUpd2.default.colors.wrap(NICK_COLORS[colorIndex], displayUsername);
      }
  ...
  sendToDiscord(author, channel, text) {
    ...
    const withMentions = withFormat.replace(/@([^\s#]+)#(\d+)/g, (match, username, discriminator) => {
      ...
      const user = guild.members.find(x => Bot.caseComp(x.user.username, username) && x.user.discriminator === discriminator);
      if (user) return user;
    ...
    if (webhook) {
      ...
      if (permissions) {
        canPingEveryone = permissions.has(_discord2.default.Permissions.FLAGS.MENTION_EVERYONE);
      }
      const avatarURL = this.getDiscordAvatar(author, channel);
      const username = _lodash2.default.padEnd(author.substring(0, USERNAME_MAX_LENGTH), USERNAME_MIN_LENGTH, '_');
